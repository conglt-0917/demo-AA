...
function getAccountDeployer(factory: SimpleAccountFactory, accountOwner: string, _salt: number = 0): string {
    return hexConcat([
      factory.address,
...
async function main() {
  ...
  let paymaster: LegacyTokenPaymaster = await new LegacyTokenPaymaster__factory(ethersSigner).deploy(factory.address, 'ttt', entryPoint.address)
  let pmAddr = paymaster.address
  console.log(`Paymaster is: ${pmAddr}`)
...
//   console.log(await getTokenBalance(paymaster, account.address))
